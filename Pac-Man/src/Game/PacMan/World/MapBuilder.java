...
public class MapBuilder {
	...
	public BufferedImage redGhost = Images.ghost[0];
	public BufferedImage pinkGhost = Images.ghost[1];
	public BufferedImage blueGhost = Images.ghost[2];
	public BufferedImage orangeGhost = Images.ghost[3];
			...
			for (int j = 0; j < mapImage.getHeight(); j++) {
				...
				if(currentPixel == boundBlock){
					...
					if (random.nextInt(30)==0) {
						BaseStatic fruit = new Fruits(xPos,yPos,pixelMultiplier-5,pixelMultiplier-5,handler,Images.fruits[random.nextInt(2)]);					
						mapInCreation.addBlock(fruit);
					} else {
						BaseStatic bigDot = new BigDot(xPos,yPos,pixelMultiplier-5,pixelMultiplier-5,handler);					
