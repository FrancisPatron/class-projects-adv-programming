...
public class Map {
	...
	public void drawMap(Graphics2D g2) {
		for (BaseStatic block:blocksOnMap) {
			if(block instanceof BigDot) {
				...
				g2.drawImage(PacManState.getBigDotBlinkAnimation(), block.x, block.y, block.width, block.height, null);
			}
		...
		for (BaseDynamic entity:enemiesOnMap) {
			if (entity instanceof PacMan) {
				...
				if(handler.getPacManState().getBlinking()==true) {
					g2.drawImage(((Ghost) entity).ghostblink.getCurrentFrame(), entity.x, entity.y, entity.width, entity.height, null);
				}
				else {
	...
	public void reset() {
		handler.getPacman().setX(126);
		handler.getPacman().setY(648);
