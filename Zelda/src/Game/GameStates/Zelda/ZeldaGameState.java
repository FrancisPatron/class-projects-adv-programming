...
public class ZeldaGameState extends State {
	...
	public Link link;
	public Enemy enemy;
	public static boolean inCave = false, enemyalive = true;
	private boolean equippedSword = false;
	...
	public ZeldaGameState(Handler handler) {
		...
		if (enemyalive) {
			enemy = new Enemy(xOffset + (stageWidth / 4), yOffset + (stageHeight / 4), Images.octorok, handler);
		}
	...
	public void render(Graphics g) {
		if (inCave) {
			...
			Rectangle swordBounds = new Rectangle((3 * (ZeldaGameState.stageWidth / 16)) + ZeldaGameState.xOffset + 232,
					(2 * (ZeldaGameState.stageHeight / 11)) + ZeldaGameState.yOffset + ((16 * worldScale)) + 100, 17,
					44);
			...
			g.drawImage(Images.oldMan, (3 * (ZeldaGameState.stageWidth / 16)) + ZeldaGameState.xOffset + 215,
					(2 * (ZeldaGameState.stageHeight / 11)) + ZeldaGameState.yOffset + ((16 * worldScale)) + 40, 50, 50,
					null);
			g.drawImage(Images.fire, (3 * (ZeldaGameState.stageWidth / 16)) + ZeldaGameState.xOffset + 115 - 50,
					(2 * (ZeldaGameState.stageHeight / 11)) + ZeldaGameState.yOffset + ((16 * worldScale)) + 40, 50, 50,
					null);
			g.drawImage(Images.fire, (3 * (ZeldaGameState.stageWidth / 16)) + ZeldaGameState.xOffset + 215 + 150,
					(2 * (ZeldaGameState.stageHeight / 11)) + ZeldaGameState.yOffset + ((16 * worldScale)) + 40, 50, 50,
					null);
			if (!equippedSword) {
				g.drawImage(Images.caveSword, (3 * (ZeldaGameState.stageWidth / 16)) + ZeldaGameState.xOffset + 232,
						(2 * (ZeldaGameState.stageHeight / 11)) + ZeldaGameState.yOffset + ((16 * worldScale)) + 100,
						17, 32 + 12, null);
				if (swordBounds.intersects(link.bounds)) {
					equippedSword = true;
				}
			}
			link.render(g);
			...
			for (int i = 0 ;i<handler.getZeldaGameState().link.health;i++) {
				g.drawImage(Images.heart, (3 * (ZeldaGameState.stageWidth / 32 + i*20)),
						(2 * (ZeldaGameState.stageHeight / 11)) + ZeldaGameState.yOffset + ((16 * worldScale)) + 40, 50, 50,
						null);
			}
		}
	...
	public void setEnemyalive(boolean enemyalive) {
		ZeldaGameState.enemyalive = enemyalive;
	}
}
