...
public class ZeldaMapMakerState extends State {
	...
	public void tick() {
		...
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_H)){
		 ...
		 /*
		 * Implement the keyboard button ​‘B’ ​that in the mapmaker (Press M in the main menu) 
		 * moves the selected tile to be the tile at the halfway point of the tileset.
		 */
		if (handler.getKeyManager().keyJustPressed(KeyEvent.VK_B)) {
			int listMid = selectedList.size()/ 2; //gets the middle index of the list.
			BufferedImage tempLoc = selectedList.get(counter); //saves the image temporarily.
			selectedList.set(counter, selectedList.get(listMid)); //puts the selected tile in the middle index.
			selectedList.set(listMid, tempLoc); //puts the tempLoc in the original spot of the image swapped.
		...
		if (handler.getKeyManager().key_randomTiles) {
			Random random = new Random();
			if (linking){
				handler.getDisplayScreen().confirm("Please click where the last tile will teleport too.");
			}else {
