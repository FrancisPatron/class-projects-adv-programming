 ...
 */
public class MovingTilesUp extends MMBaseEntity {
	public MovingTilesUp (int x, int y, BufferedImage sprite, Handler handler) {
	...
	public void tick() {
		if (((ZeldaMMGameState)handler.getState()).map.link.interactBounds.intersects(bounds)){
			...
			moveLink(Direction.UP, tilesLenght);
		}
	}
	...
	public void moveLink(Direction direction,int distance) {
		if (this.y-distance*this.height!=((ZeldaMMGameState)handler.getState()).map.link.y) {
	...
	public int checkAdjacentTiles(int x, int y) {
		int number = 1;
		if (isAMoveTile(x,y-this.height)) {
			number++;
			checkAdjacentTiles(x, y-this.height);
			return number;
		} else {
			return number;
		}
