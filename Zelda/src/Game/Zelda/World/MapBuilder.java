...
import Game.Zelda.Entities.Statics.MovingTilesDown;
import Game.Zelda.Entities.Statics.MovingTilesLeft;
import Game.Zelda.Entities.Statics.MovingTilesRight;
import Game.Zelda.Entities.Statics.MovingTilesUp;
import Main.Handler;
...
public class MapBuilder {
	...
	public static Map createMap(BufferedImage mapImage, Handler handler,String name){
		...
		for (int i = 0; i < mapImage.getWidth(); i++) {
			for (int j = 0; j < mapImage.getHeight(); j++) {
				...
				}else if(currentPixel == up) {
					MovingTilesUp ghost = new MovingTilesUp(xPos, yPos, Images.moveTiles.get(0), handler);
					mapInCreation.addBlock(ghost);
				}else if(currentPixel == down) {
					MovingTilesDown ghost = new MovingTilesDown(xPos, yPos, Images.moveTiles.get(3), handler);
					mapInCreation.addBlock(ghost);
				}else if(currentPixel == left) {
					MovingTilesLeft ghost = new MovingTilesLeft(xPos, yPos, Images.moveTiles.get(2), handler);
					mapInCreation.addBlock(ghost);
				}else if(currentPixel == right) {
					MovingTilesRight ghost = new MovingTilesRight(xPos, yPos, Images.moveTiles.get(1), handler);
					mapInCreation.addBlock(ghost);
				} 
	...
	//moving tiles
	public static int up = new Color(5, 111, 200).getRGB();
	public static int down = new Color(10, 200, 111).getRGB();
	public static int left = new Color(20, 150, 47).getRGB();
	public static int right = new Color(16, 32, 166).getRGB();
